...
class RepositoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testIsAddingArrayOfFiles()
    {
        ...
        $repository->add(['test_file7.txt', 'test_file8.txt', 'test_file9.txt']);
    ...
    public function testIsGettingCurrentBranch()
    {
        ...
        $this->assertContains('(HEAD detached at', $new_branch);
    ...
    public function testIsGettingCommits()
    {
        ...
        foreach ($commits as $commit) {
            ...
            $this->assertInstanceOf('Gitlib\Model\Commit\Commit', $commit);
            $this->assertEquals($commit->getMessage(), 'The truth unveiled');
            $this->assertInstanceOf('Gitlib\Model\Commit\Author', $commit->getAuthor());
            ...
            $this->assertEquals($commit->getParentsHash(), []);
    ...
    public function testIsGettingTree()
    {
        ...
        foreach ($files as $file) {
            ...
            $this->assertInstanceOf('Gitlib\Model\Blob', $file);
    ...
    {
        $repository = $this->client->getRepository(self::$tmpdir.'/testrepo');
        $blob = $repository->getBlob('master:MyFolder/crazy.php')->getContent();
        $this->assertEquals('Lorem ipsum dolor sit amet', $blob);

        $blob = $repository->getBlob('master:test_file4.txt')->getContent();
    ...
    public function testIsGettingCommit()
    {
        ...
        foreach ($commits as $commit) {
            ...
            $this->assertInstanceOf('Gitlib\Model\Commit\Commit', $singleCommit);
            $this->assertInstanceOf('Gitlib\Model\Commit\Author', $singleCommit->getAuthor());
    ...
    public function testIsGettingBlame()
    {
        ...
        $this->assertEquals($repository->getBlame('original_file.txt'), []);
    }
    ...
    {
        $statisticsMock = \Mockery::mock('Gitlib\Statistics\StatiticsInterface');
        ...
        $this->assertEquals(
            [strtolower(get_class($statisticsMock)) => $statisticsMock],
            $repo->getStatistics(),
